...
$i=0;
$fileExt='';
$referenceshare = $_POST['referenceshare'];
$count = strlen($referenceshare)-1;
while($referenceshare[$count]!='.')
{
   $fileExt[$i]=$referenceshare[$count];
   $count--;
   $i++;
}
$fileActualExt = strrev($fileExt);
//echo $fileExt;

//echo $referenceshare;

$fileTmpName = $referenceshare;
               ...
               fclose($myfile);
                       ...
                       echo "<table>
                         <tr>
                           ...
                           <td style='padding: 15px;'><button id='".$createimgfile."' onclick='snapcmd(this.id)' class='btn btn-sm btn-outline-secondary' style='background-color: transparent !important; border: none;    cursor:pointer;    overflow: hidden;    outline:none;' type='button' name='button'><i class='far fa-comments'></i>  comment</button></td>
